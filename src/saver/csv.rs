...
use crate::{
    data::data_field::{EFieldType, FieldInfo},
    utils,
};
    ...
    fn output(
        ...
        for one in info {
            ...
            if one.header.len() > 0 {
                ...
                let mut field_name = one.name.clone();
                let f1 = field_type.contains(",")
                    || field_type.contains("\r")
                    || field_type.contains("\n")
                    || field_type.contains("\'")
                    || field_type.contains("\"");
                let f2 = out_type == "scsv";
        ...
        for row in data.iter().sorted_by_key(|a| utils::map_get_i32(*a, key)) {
            ...
            for one in info {
                ...
                let mut flag = false;
                if out_type == "scsv" {
                    tmp = v.trim().replace("'", "\\'").replace("\"", "\"\"");
                    flag = one.is_array
                        || one.val_type == EFieldType::Str
                        || one.val_type == EFieldType::Table;
