...
impl AddAction {
    pub fn new(
        target: &HashMap<String, DataTable>,
        table_name: &str,
...
impl ImportAction {
    fn _new(target: &HashMap<String, DataTable>, path: PathBuf, tab: String) -> Result<Self> {
        ...
        if table.is_none() {
            bail!(error::AppError::SheetNotFound(tab))
        };
        let table = table.unwrap();
        ...
        if !flag {
            bail!(error::AppError::ImportExcelKeyNotFound(
...
impl CopyAction {
    pub fn new(
        ...
        for one in child {
            ...
            let copy_list = data_table.get_show_name_list(
