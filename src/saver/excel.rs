...
impl DataSaver for ExcelSaver {
    fn output(
        ...
        for one in info {
            ...
            max_col = max_col + 1;
            let cell_name =
                umya_spreadsheet::helper::coordinate::coordinate_from_index(&max_col, &1);
            let cell = sheet.get_cell_mut(&cell_name);
            cell.set_value(&one.title);

            let cell_name =
                umya_spreadsheet::helper::coordinate::coordinate_from_index(&max_col, &2);
            let cell = sheet.get_cell_mut(&cell_name);
            cell.set_value(&one.origin);

            let cell_name =
                umya_spreadsheet::helper::coordinate::coordinate_from_index(&max_col, &3);
            let cell = sheet.get_cell_mut(&cell_name);
